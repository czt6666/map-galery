 ...
 * @param {string} extensions - 返回结果控制，默认"base"，可选值："base"（基本信息）或"all"（包含详细信息）
 * @returns {string} - 返回构建的 API URL
 */
function buildApiUrl(gpsLocation) {
    return `https://restapi.amap.com/v3/geocode/regeo?key=${config.apiKey}&location=${gpsLocation}&extensions=${config.extensions}`;
}
