...
// 压缩图片的函数 
async function imageCompress(imagePath, outputFile) {
    const newFileName = changeFileExtName(path.basename(imagePath)); // 新文件名
    const relativeDir = truncatePath(imagePath);
...
// 截取路径的前半部分
function truncatePath(absolutePath) {
    const prefix = config.inputPath.replace(/\\/g, '/')
...
function getResizeAndComposite(width, height, orientation) {
    ...
    // 计算水印位置
    const topOffset = getResizeHeight(width, height, resizeWidth) - config.watermark.size[(isVertical ? 'w' : 'h')] - config.watermark.padding;
    const leftOffset = isVertical ? config.watermark.padding : resizeWidth - config.watermark.size.w - config.watermark.padding;
    // 水印图片路径
